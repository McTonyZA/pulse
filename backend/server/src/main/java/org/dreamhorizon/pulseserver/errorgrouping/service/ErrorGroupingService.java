...
import static org.dreamhorizon.pulseserver.errorgrouping.FramesParser.*;
...
public class ErrorGroupingService {
    ...
    public static String traceIdHex(ByteString bs) {
        if (bs == null || bs.isEmpty()) return null;          // absent
        byte[] b = bs.toByteArray();
        if (b.length != 16) return null;                      // invalid length
        return toHex(b);
    }
    ...
    public static Lane choosePrimary(ParsedFrames st) {
        ...
        if (js == 0 && jv == 0 && nk == 0) return Lane.UNKNOWN;
        ...
        if (st.getPrimaryExceptionLane() != null) {
            ...
            if (primary == Lane.JS && js > 0) return Lane.JS;
            if (primary == Lane.JAVA && jv > 0) return Lane.JAVA;
            if (primary == Lane.NDK && nk > 0) return Lane.NDK;
        }
        
        // Priority 2: Total frame count (tie-breaker: JS > JAVA > NDK)
        int max = Math.max(js, Math.max(jv, nk));
        if (js == max) return Lane.JS;
        if (jv == max) return Lane.JAVA;
        return Lane.NDK;
    }
    ...
    public static List<Frame> selectPrimaryTokens(ParsedFrames st, Lane lane, int topN) {
        ...
        for (Frame f : frames) { if (f.isInApp()) { chosen.add(f); if (chosen.size() == topN) break; } }
        if (chosen.isEmpty()) {
            for (Frame f : frames) { chosen.add(f); if (chosen.size() == topN) break; }
        }
        return chosen;
    }
}
