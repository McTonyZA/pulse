...
public class ErrorGroupingUtils {
    ...
    public static int safeInt(String s) { try { return Integer.parseInt(s); } catch (Exception e) { return -1; } }
    ...
    public static boolean isJavaInApp(String classFqcn) {
        if (classFqcn == null) return false;
        for (String p : JAVA_INAPP_PREFIXES) if (classFqcn.startsWith(p)) return true;
        return false;
    }

    public static boolean isJsInApp(String file) {
        String f = file.replace('\\', '/');
        for (String s : JS_OUT_OF_APP_CONTAINS) if (f.contains(s)) return false;
        return true;
    }
    ...
    public static String normalizeJsFunction(String fn) {
        if (fn == null) return "anonymous";
        String f = fn.trim();
        if (f.startsWith("bound ")) f = f.substring("bound ".length());
        if (f.isEmpty() || "<anonymous>".equals(f)) f = "anonymous";
        return f;
    }
    ...
    public static String sanitizeJsFile(String file) {
        ...
        int h = f.indexOf('#'); if (h >= 0) f = f.substring(0, h);
        return f;
    }
}
